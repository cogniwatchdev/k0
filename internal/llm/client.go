...
// buildSystemLean returns ONLY the task instruction — no soul prompt.
// Used for JSON-only calls where the soul context is wasted tokens.
func (c *Client) buildSystemLean(taskInstruction string) string {
	return taskInstruction
}
...
// Uses the full soul prompt for rich, contextual responses.
...
func (c *Client) complete(ctx context.Context, system, prompt, format string, temp float64, maxTokens int) (string, error) {
	fullSystem := system // already built by caller
...
// ─── Goal planning (conversational) ───────────────────────────────────────

// PlanGoal asks the LLM to produce a human-readable plan for the goal.
// Returns a JSON object the TUI can render for user confirmation.
func (c *Client) PlanGoal(ctx context.Context, goal string) (string, error) {
...
// extractJSONObject scans the raw LLM output for the first {...} JSON object.
